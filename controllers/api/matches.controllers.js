...
module.exports.createMatch = function (req, res) {
  ...
  util.getCurrentUser(req).then((user) => {
    if (user) { // if the endpoint is hit from employer CMS
      initiator_id = user._id;
      // update carrots

      session_carrot_stats = user.carrots;
      // console.log(req.session.carrots)
      updated_carrot_stats = _updateCarrotStats(user._id, session_carrot_stats, carrots_used);
        ...
        Match.findOneAndUpdate(matchQuery, newMatch, { upsert: true }).then(matchdone => {
          Match.findOne(matchQuery).then(match => {
            ...
            Worker.findById(worker_id).exec(async function (err, worker) {
              ...
              // respond
              if (user._id) { // respond a redirect for employerCMS request
